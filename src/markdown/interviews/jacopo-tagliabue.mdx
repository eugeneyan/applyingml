...
Outside of product-driven projects, I like to paraphrase the great [Steven Levitt](https://en.wikipedia.org/wiki/Steven_Levitt): ~~economics~~ ML has excellent tools for gaining answers but a shortage of interesting questions. Good ML questions often rely on the ability to collect, find or generate unexpected data, more than small algorithmic tweaks: with my friend [Giovanni](https://giovannicassani.github.io/), we have been lately working on quantifying decision biases in online election systems and we will (hopefully) publish our results soon. 
...
Teams that work are small (a casual observer would say “understaffed”), focused and own the work 100%: we subscribe to the “end-to-end data scientist” view, and encourage younger teammates to get curious about the use case, not just the tech.
...
We use Metaflow for both research and production pipeline, SaaS products for experiment tracking, and PaaS offering for quick serving: building a working prototype is much better than slides, and in today’s cloud offerings there are plenty of one-liners to set up an endpoint from a model artifact produced with Metaflow. 
