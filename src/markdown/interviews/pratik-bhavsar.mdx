...
We interact with backend engineers to fix up the contract and configuration but MLE does the full ML deployment. MLE are also expected to do system and load tests if needed.

Every time we deploy a new feature we post it out on slack #product channel and get it tested. 

Previously I worked on a large team with backend, DevOps, testing engineer, manager, scrum master and PM. Over there the isolation of work was higher and my work ended at writing the server code.
...
GrafanaGraphana is quite helpful for monitoring and alerts.
