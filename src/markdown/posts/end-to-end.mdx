...
There's a worrying trend in the field of data science and machine learning where the ML process (problem framing, data engineering, ML, deployment, maintenance) is split across different people. It leads to coordination overhead, diffusion of responsibility, and lack of a big picture view. 

IMHO, data scientists can be more effective by becoming _more_ end-to-end. Here, I'll discuss the benefits, counter-arguments, and the experiences of Stitch Fix amakend Netflix.

## From start (identify the problem) to finish (solve it)

You may have come across similar _labels_ and definitions, such as:
...
<details><summary>Diffusion of Responsibility & Social Loafing</summary>
...
Assess each model’s performance (e.g., query latency, memory footprint) before picking one and writing a basic [inference class]({{ site.baseurl }}{% link posts/_posts/2017-02-13-product-categorization-api-part-3-creating-an-api.md %}#creating-a-titlecategorize-class) around it for production (`software engineering`). (You might also want to build a simple user interface). Then, containerise and deploy it online for others to use via your preferred cloud provider (`dev ops`).
...
<FigureCaption caption="Full cycle dev are empowered across the entire software life cycle" source="https://netflixtechblog.com/full-cycle-developers-at-netflix-a08c31f83249" />
