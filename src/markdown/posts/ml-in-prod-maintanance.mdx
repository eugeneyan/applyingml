...
In the [previous post](/resources/ml-production-challenges/), we discussed the challenges faced after deploying machine learning, including: 
- Unexpected data schema changes
- Unwanted model interactions
- Messy infra and codebase
- Real-world bias and adversaries
- Organization issues and diffusion of responsibility
- Operations support for customers

This guide will share some practices I’ve found useful to maintaining machine learning in production. With close to 20 suggestions, it can get overwhelming. Fret not, I’ll highlight the top three **must-haves** and **good-to-haves** at the end of this post.
...
<FigureCaption caption="This SHAP summary plot shows sex, passenger class, and age were most important as features." />
...
(An aside: When measuring _online_ performance, focus on what your system can have a direct impact on, such as clicks, purchases, bounce rate. Consider indirect metrics (e.g., user return rate, daily average users) _only_ after you grok the direct metrics. Avoid delayed and big-picture metrics (e.g., user satisfaction, traffic growth)—there could be multiple confounding factors for these.)
